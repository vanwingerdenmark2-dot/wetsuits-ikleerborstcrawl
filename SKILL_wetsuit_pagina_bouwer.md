...
**Exacte maattabel zelf opzoeken — verplicht.** Zoek de officiële maattabel van het specifieke merk/model op en neem de cijfers voor de aangeboden maat letterlijk over in de Pasvorm-kaart én de Maat-spec (lengte, gewicht, en borst/buste als het merk dat geeft). Niet schatten of een andere maat overnemen. Link naar de officiële maat-/pasvormpagina van het merk, niet naar een generieke collectiepagina.
...
**Specs zélf opzoeken — verplicht.** Mark hoeft geen specificaties of maattabellen aan te leveren. Zoek per pak zelf de officiële specs op (eerst de merksite, anders betrouwbare retailers zoals ProSwimwear, SportsShoes, Aqua Swim Supplies). Vul de specs- en features-secties met wat je daar vindt. **Alleen verifieerbare feiten — niets verzinnen, niets afleiden uit de modelnaam als je het niet zeker weet** (zie anti-patroon 11). Kun je een veld niet vinden/verifiëren: rij weglaten. Let op modelvarianten (bv. "Alta" vs "Alta Thermal") — pak de juiste.
